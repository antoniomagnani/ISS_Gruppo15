...
protected LindaLike(IOutputView aview, String spaceName){
...
public static synchronized ILindaLike initSpace(IOutputView aview, String spaceName){
// if( space == null ){
	LindaLike myspace = new LindaLike(aview,spaceName);
	space = myspace;
	myspace.doprintln(" ******************************************** ");
	myspace.doprintln(" LindaLike: init and cleans a singleton space " + spaceName);
	myspace.doprintln(" ******************************************** ");
	myspace.clean();
// }
 return space;
...
public static synchronized ILindaLike getSpace(){
	if( space == null ){
		LindaLike myspace = new LindaLike(null,"defaultSpace");
		myspace.doprintln(" ******************************************** ");
...
public static synchronized ILindaLike getSpace(IOutputView view){
	if( space == null ){
		LindaLike myspace = new LindaLike(view,"defaultSpace");
		space = myspace;
		myspace.doprintln(" **************************************** ");
...
//	if( queries.size() == 0 ) wait(); //perhaps some remote signal will appear
	...
	while( iter.hasNext() ){
		...
		if( queryMsg.getMsgName().contains("_")){ //is not a signal
			 ...
			 query = "mmm(" + queryMsg +"," + "MsgNum),MsgNum>"+ lastMsgRdMemo.get(queryMsg.msgId()+subjName) ;
		}
		...
		SolveInfo sol = engine.solve(query+".");
	   	if( sol.isSuccess() ){
	   			...
	   			lastMsgRdMemo.put(queryMsg.msgId(), N);
	   			Term msg		= ((Struct)content).getArg(0); //content=mmm( A, N) quindi prendo A
...
/* --------------------------
	OTHER UTILITIES
 ---------------------------- */
 	protected String buildMsgStr( /* EXPAND MsgParams FOR msgStruct-» */ ){
