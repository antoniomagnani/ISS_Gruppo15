...
public class ConnProtOut implements ILindaLike {
	...
	public ConnProtOut(String receiver, String msgId, String sender,
		...
		//ADDED TO HANDLE ROW MESSAGES
		rt = new ReceiveThread(sender,connection);
		rt.start();
	}
	...
	public IConnInteraction connect() throws Exception {
		...
		if (conn == null) {
			...
			if (conn != null && ! RunTimeKb.worksWith( "HTTP", receiver, msgId) ){
				...
				rt = new ReceiveThread(receiver,conn);
				rt.start();
...
class ReceiveThread extends ConnInputReceiver {
	...
	public ReceiveThread(String receiver,IConnInteraction connection) throws Exception{
		super("ReceiveThread",connection,null);
		this.receiver = receiver;
	...
	public void resetRawMsg() throws Exception{
		System.out.println("*** resetRawMsg");
		rowMsg = false;
	}
 	public boolean receiveRawCompleted( ) throws Exception{ 	
	...
	public synchronized RawBuffer receiveRaw( ) throws Exception{
		...
		//System.out.println("receiveRaw " + m );
...
//			println("received " + mm );
		...
		if( answer != null ){
		if( mm.msgContent().startsWith("internalMsg_rowComm")){
			//Send a dispatch row done on the same conn
